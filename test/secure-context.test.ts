...
describe.each([
  ...
  'Secure Context',
  (
    aliceGenerator: () => Promise<CryptoKeyPair>,
    bobGenerator: () => Promise<CryptoKeyPair>
  ) => {
    ...
    [
      ...
      it(`should resolve path on multiple objects: ${expected}, ${path}`, async () => {
        const child = {
          a: {
            b: {
              c: arg ?? expected,
    ...
    it('should create share metadata', async () => {
      ...
      const cose: Cose = await scidToCose(ipfs, scid, codec);

      const { content } = await decrypt(cose, alice.privateKey!);
    ...
    it('should create recursive share metadata', async () => {
      ...
      const { content: childContent } = await decrypt(
        translate(childCose.value),
        alice.privateKey!
      );
    ...
    it('should get content from unecrypted Cose', async () => {
      const cid = await secure.put(SAMPLE_JSON);
    ...
    it('should store/load complex object', async () => {
      ...
      expect(
        (await bobIpfs.get(scid, { path: 'document/0/guid' })).value
      ).toStrictEqual('84629167-e6d3-4a00-a501-8aa65656656e');
      expect(
        (await bobIpfs.get(scid, { path: 'document/2/guid' })).value
      ).toStrictEqual('e60a90e1-1a8e-4b8c-ada6-57230d0d60b4');
    ...
    describe('Deterministic CID', () => {
      it('Content CID should be deterministic', async () => {
        ...
        expect(cid1).toStrictEqual(cid2);
      });

      it('Content CID should not be deterministic', async () => {
        const cid1 = await secure.put(SAMPLE_JSON);
        const ctx2 = await SecureContext.create({
          ...alice,
          deterministicCID: false,
        });
        const secure2 = ctx2.secure(ipfs);

        const cid2 = await secure2.put(SAMPLE_JSON);

        expect(cid1).not.toStrictEqual(cid2);
      });
