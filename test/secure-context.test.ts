...
import { Cose, SecureContextConfig } from '../src/types';
...
describe.each([
  ...
  'Secure Context',
  (aliceGenerator: () => Promise<SecureContextConfig>, bobGenerator: () => Promise<SecureContextConfig>) => {
    let alice: SecureContextConfig;
    let bob: SecureContextConfig;
    ...
    it('should create recursive share metadata', async () => {
      ...
      const { content: childContent } = await decrypt(translate(childCose.value), alice.privateKey!);
