...
export class SCID {
  ...
  static async from(cid: string | CID): Promise<SCID> {
    ...
    if (sharedCID.code !== identity.code) {
      throw new Error(
        `Unexpected CID codec. Expected: ${identity.code}, actual: ${sharedCID.code}`
      );
    }
    if (sharedCID.byteLength !== 84) {
      throw new Error(
        `Unexpected CID length. Expected: 84, actual: ${sharedCID.byteLength}`
      );
    }
    ...
    const contentCID = CID.decode(
      sharedCID.bytes.subarray(cidStart, cidStart + CID_BYTES)
    );
    const rawIV = sharedCID.bytes.subarray(ivStart, ivStart + IV_BYTES);
    const importedKey = await importRawAESGCMKey(
      sharedCID.bytes.subarray(keyStart, keyStart + KEY_BYTES)
    );
  ...
  async asCID(): Promise<CID> {
    const digest = await identity.digest(
      concat(
        this.cid.bytes,
        this.iv,
        new Uint8Array(await exportRawKey(this.key))
      )
    );
